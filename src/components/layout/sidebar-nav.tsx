    ...
    <>
      ...
      {user && (
        ...
        >
          ...
          {/* Admin con plan activo: link Mi Suscripción expandido */}
          {isAdmin && planActivo && isOpen && (
            <div className="px-3 pb-4">
              <Link
                href="/configuracion/suscripcion"
                className={cn(
                  "flex items-center rounded-lg text-sm font-medium transition-all duration-200 px-4 py-3",
                  pathname === '/configuracion/suscripcion'
                    ? "bg-blue-100 text-blue-900 dark:bg-blue-900/40 dark:text-blue-100 shadow-sm"
                    : "text-slate-700 hover:bg-slate-100 dark:text-slate-300 dark:hover:bg-slate-800"
                )}
                onClick={() => setIsMobileOpen(false)}
              >
                <Crown className="h-5 w-5 text-yellow-500 mr-3 shrink-0" />
                <span className="truncate font-medium">Mi Suscripción</span>
              </Link>
            </div>
          )}

          {/* Admin con plan activo: icono colapsado */}
          {isAdmin && planActivo && !isOpen && (
            <div className="px-3 pb-4">
              <Button
                variant="ghost"
                size="icon"
                className="w-full h-10 group relative"
